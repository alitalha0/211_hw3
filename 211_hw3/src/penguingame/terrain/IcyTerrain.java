...
public class IcyTerrain {
    ...
    for (ITerrainObject obj : objectsAtNext) {
        if (obj instanceof IHazard) {
            ...
            // Note: handleCollision often handles placement/sliding, so we only add
            // if the penguin is still active and hasn't been moved by the collision handler.
    ...
    public void slide(ITerrainObject slider, Direction dir) {
        ...
        while (true) { 
            ...
            List<ITerrainObject> objectsAtNext = getObjectsAt(next);
            // if next tile is empty, then it is safe to move directly by settin current position to next
            if (objectsAtNext.isEmpty()) {
                current = next;
    ...
    private void handleCollision(ITerrainObject slider, ITerrainObject blocker, Direction dir, Position sliderPos) {
        // FIX: Penguin moves TO the food square, then collects it.
        if (slider instanceof Penguin && blocker instanceof Food) {
            Penguin p = (Penguin) slider;
            Food f = (Food) blocker;
            
            // 1. Visually move penguin to the food's position
            removeObject(p);
            addObject(p, blocker.getPosition());
            
            // 2. Collect food and remove it from board
            p.collectFood(f);
            removeObject(f);
            
            // 3. Return immediately (stops sliding because we returned from handleCollision)
            return;
        }
        ...
        // ... (Keep the rest of your existing logic for other collisions)
