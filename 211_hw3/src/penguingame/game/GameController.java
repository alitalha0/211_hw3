...
public class GameController {
    ...
    private void playAITurn(Penguin penguin) {
        ...
        // SRockhopper automatically uses ability when moving toward hazards
        if (penguin instanceof RockhopperPenguin && !penguin.isSpecialActionUsed()) {
            ...
            if (!hazardDirs.isEmpty()) {
                // Rockhopper will automatically use jump ability because there is a hazards on the way
                useSpecial = true;
        ...
        // For other penguins, there is a 30% chance to use special action
        else if (!penguin.isSpecialActionUsed() && random.nextDouble() < 0.3) { // if special actin is not used and applying %30 chance
            useSpecial = true;                      
        ...
        // Chooses direction if it is not already chosen by Rockhopper logic
        if (chosenDir == null) {
            chosenDir = chooseAIDirection(penguin);
        }
        
        // Announces the chosen direction
        System.out.println(penguin.getName() + " chooses to move " + getDirectionName(chosenDir) + ".");
        
        // Executes the special action if needed
        if (useSpecial) {
        ...
        // Executes the normal slide
        penguin.slide(chosenDir, terrain);
        
        // Handlse any results of the movement
        handlePositionAfterMove(penguin);
    }
